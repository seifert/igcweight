...
def error_message_dialog(parent, message, exception=None):
    """
    error_message_dialog(Window parent, str message, Exception
    exception=None) - show error message dialog
    """
...
def info_message_dialog(parent, message):
    """
    info_message_dialog(Window parent, str message) - show information
    message dialog
    """
    wx.MessageBox(
        message, _("Information"), wx.OK | wx.ICON_INFORMATION, parent)


def GetPhotoBitmap(max_size, photo=None):
    """
    GetPhotoBitmap(self, Size max_size, Photo photo=None) -> Bitmap -
    load photo and return bitmap
    """
    ctrl_w, ctrl_h = max_size
    key_name = (
        photo is not None and
        '%dx%d-%s' % (ctrl_w, ctrl_h, photo.md5) or
        'empty-photo')
    bitmap = None

    if key_name not in _thumbnails_cache:
        # Not cached, create bitmap from file
        if photo is not None:
            thumbnail_path = os.path.join(
                settings.IMG_CACHE_DIR, '%s.jpg' % key_name)
            if not isfile(thumbnail_path):
                ...
                if image_w > ctrl_w or image_h > ctrl_h:
                    ...
                    image.Rescale(
                        image_w, image_h, quality=wx.IMAGE_QUALITY_HIGH)
            ...
            # Load empty photo
            image = wx.Image(
                os.path.join(settings.IMAGES_DIR, 'lphoto.png'),
                type=wx.BITMAP_TYPE_PNG)
            image_w, image_h = image.GetSize()

        # Resize photo thumbnail
        image.Resize(
            (ctrl_w, ctrl_h), ((ctrl_w-image_w) / 2, (ctrl_h-image_h) / 2))
...
class VirtualListCtrl(wx.ListCtrl):
    ...
    def __init__(self, parent, id):
        wx.ListCtrl.__init__(
            self, parent=parent, id=id, style=wx.LC_REPORT | wx.LC_VIRTUAL |
    ...
    def InsertColumn(self, col, heading, fieldname,
                     format=wx.LIST_FORMAT_LEFT, proportion=1):
        """
        InsertColumn(self, long col, str heading, str fieldname,
                     int format=wx.LIST_FORMAT_LEFT, int proportion=0)
        """
        if proportion < 1:
            raise ValueError("'width' attribute must be least then 0")
        self.__columns[col] = {
            'proportion': proportion, 'fieldname': fieldname}
    ...
    def OnGetItemText(self, item, col):
        """
        OnGetItemText(self, int item, int col) -> str - get column data
        from datasource
        """
        colname = self.__columns[col]['fieldname']
        if self.GetItemTextMethod is not None:
    ...
    def OnGetItemAttr(self, item):
        """
        OnGetItemAttr(self, item) -> wx.ListItemAttr - get and return
        item attr
        """
        if self.GetItemAttrMethod is not None:
